...
test.describe('Login to Existing User Before Checking Out Tests', () => {
	...
	test('After logging in the home page displayes the logged in user.', async ({ page }) => {
		await page.goto('https://automationexercise.com');
		await page.getByRole('link', { name: ' Signup / Login' }).click();
	...
	test('After logging in the user adds products to their cart they are able to complete an order and view place order page.', async ({ page }) => {
		await page.goto('https://automationexercise.com');
		await page.getByRole('link', { name: ' Signup / Login' }).click();
		await page.locator('[data-qa="login-email"]').fill(userEmailAddress);
		await page.locator('[data-qa="login-password"]').fill(userPassword);
